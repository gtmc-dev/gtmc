...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  return (
    <div className="p-6 sm:p-8 pb-32 min-h-screen bg-transparent relative border border-tech-main/40 backdrop-blur-sm">
      ...
      <div className="absolute top-6 right-6 sm:top-8 sm:right-8 group z-20 flex flex-col items-end gap-2">
        <Link href={`/draft/new?file=${encodeURIComponent(editPath)}`}>
          <button className="flex items-center gap-2 border border-tech-main/40 bg-tech-main/10 hover:bg-tech-main text-tech-main hover:text-white px-4 py-2 font-mono text-xs uppercase tracking-widest transition-all duration-300 relative overflow-hidden min-h-[44px]">
            <span className="relative z-10 font-bold">[EDIT_TARGET]</span>
          </button>
        </Link>
        <div className="text-xs font-mono text-tech-main flex items-center gap-2 opacity-80 transition-opacity hover:opacity-100">
