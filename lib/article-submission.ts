...
export async function resolveArticleFilePath(
  ...
  const candidates = normalizedPath.endsWith(".md")
    ...
    : [normalizedPath, `${withoutExtension}.md`, `${withoutExtension}/README.md`];
  ...
  return normalizedPath.endsWith(".md") ? normalizedPath : `${withoutExtension}.md`;
}
...
export async function openDraftPullRequest({
  ...
  const resolvedFilePath = await resolveArticleFilePath(filePath, [baseMainSha, latestMainSha], token);
  ...
  const baseSnapshot = await getFileSnapshot(resolvedFilePath, baseMainSha, token);
  const latestSnapshot = await getFileSnapshot(resolvedFilePath, latestMainSha, token);
...
export async function resolveDraftSyncConflict({
  ...
  const latestMainSha = await getMainBranchHeadSha(token);
  const resolvedFilePath = await resolveArticleFilePath(filePath, [latestMainSha], token);
  ...
  if (syncedMainSha && syncedMainSha !== latestMainSha) {
    const previousMainSnapshot = await getFileSnapshot(resolvedFilePath, syncedMainSha, token);
    const latestMainSnapshot = await getFileSnapshot(resolvedFilePath, latestMainSha, token);
    const mergeResult = mergeArticleContent({
