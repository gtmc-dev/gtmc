...
因此，我们需要一种专门帮助特定方块存储数据，实时处理逻辑，进行额外渲染的组件。我们叫它们**_方块实体_**。
方块实体相为普通方块提供了三个重要的额外功能： **_使用NBT存储数据_**、**_自定义的渲染行为_** 和 **_在每一次tick中进行更新_**。
...
先看**_输出_**...
**_输出_**输出操作由 `insert()` 方法完成，它的作用是将物品从漏斗传输到目标容器，并返回一个布尔值来表示是否成功传输。如果传输成功，则漏斗进入冷却状态。
...
到此，输出的部分已经完成，接下来，我们来看看**_拉取_**的部分。
拉取物品的逻辑与输出类似，但方向相反：
